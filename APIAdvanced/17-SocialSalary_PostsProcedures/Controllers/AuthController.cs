...
namespace SocialSalary.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {
            string sqlGetUserId = "SELECT UserId FROM TutorialAppSchema.Auth WHERE UserId = "
        ...
        public IActionResult Register(UserForUpdatePasswordDto userForUpdatePasswordDto)
        {
            if (userForUpdatePasswordDto.Password == userForUpdatePasswordDto.PasswordConfirm && userForUpdatePasswordDto.Password != null && userForUpdatePasswordDto.Email != null && userForUpdatePasswordDto.OldPassword != null)
            {
                string sqlGetLoginConfirm = "TutorialAppSchema.spLoginConfirmation_Get @Email = '"
                    + userForUpdatePasswordDto.Email.ToLower() + "'";

                UserLoginConfirmDto loginConfirm = _dapper.LoadDataSingle<UserLoginConfirmDto>(sqlGetLoginConfirm);

                if (loginConfirm.UserId.ToString() != User.FindFirst("userId")?.Value)
                {
                    return StatusCode(401, "You can only change the password on your own account!");
                }

                if (loginConfirm.PasswordSalt != null && loginConfirm.PasswordHash != null)
                {
                    string oldPasswordSaltPlusString = _config.GetSection("AppSettings:PasswordHash").Value + Convert.ToBase64String(loginConfirm.PasswordSalt);

                    byte[] oldPasswordHash = _baseHelper.GetHash(userForUpdatePasswordDto.OldPassword, oldPasswordSaltPlusString);

                    for (int i = 0; i < oldPasswordHash.Length; i++)
                    {
                        if (oldPasswordHash[i] != loginConfirm.PasswordHash[i])
                        {
                            return StatusCode(401, "Authentication Failed");
                        }
                    }
                    ...
                    byte[] passwordHash = _baseHelper.GetHash(userForUpdatePasswordDto.Password, passwordSaltPlusString);
                    ...
                    SqlParameter emailParam = new SqlParameter("@EmailSupplied", SqlDbType.NVarChar, 50);
                    emailParam.Value = userForUpdatePasswordDto.Email;

                    sqlParams.Add(emailParam);

                    SqlParameter userIdParam = new SqlParameter("@UserIdSupplied", SqlDbType.Int);
                    userIdParam.Value = loginConfirm.UserId;
                    ...
                    throw new Exception("Updating Password failed on save");
                }
                throw new Exception("Original Password Salt Missing or corrupted");
            }
            throw new Exception("Passwords do not match!");
        }
    }
